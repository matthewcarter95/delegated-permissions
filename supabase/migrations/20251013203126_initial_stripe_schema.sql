/*
  ...
  3. Security
    ...
    - Implements open policies for all authenticated users to view all data
*/
...
CREATE POLICY "Authenticated users can view all customer data"
  ON stripe_customers
  FOR SELECT
  TO authenticated
...
CREATE POLICY "Authenticated users can view all subscription data"
...
CREATE POLICY "Authenticated users can view all order data"
  ON stripe_orders
  FOR SELECT
  TO authenticated
  USING (deleted_at IS NULL);

-- View for user subscriptions (open to all authenticated users)
...
-- View for user orders (open to all authenticated users)
